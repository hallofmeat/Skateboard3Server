using FluentAssertions;
using Skateboard3Server.Blaze.Tickets;
using Xunit;

namespace Skateboard3Server.Blaze.Tests.Tickets;

public class Ps3TicketParserFacts
{
    [Fact]
    public void Parse_PSN_V20_Ticket()
    {
        var trueTicket = new Ps3Ticket
        {
            Header = new TicketHeader
            {
                MajorVersion = 2,
                MinorVersion = 0
            },
            Body = new TicketBody
            {
                Length = 0xA4,
                SerialId = new byte[]
                {
                    0x79, 0x3C, 0xF6, 0xFB, 0x7A, 0xAE, 0x34, 0xCC, 0x2D, 0x47, 0x16, 0xDC, 0xA7, 0x98, 0xFD, 0xCF,
                    0x66, 0xEE, 0xE0, 0x10
                },
                IssuerId = 0x00000100,
                IssuedDate = 1350156436464,
                ExpireDate = 1350242836000,
                UserId = 6426198810416408665,
                Username = "testuser",
                Region = "fr",
                Language = 2,
                Domain = "c9",
                ServiceId = new byte[]
                {
                    0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31, 
                    0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00
                },
                Status = 2,
                DateOfBirth = null!,
                Age = 24
            },
            Footer = new TicketFooter
            {
                KeyId = new byte[] {0x34, 0xCD, 0x3C, 0xA9},
                Signature = new byte[]
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00
                }
            },
            RawBody = new byte[]
            {
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x79, 0x3C, 0xF6, 0xFB, 0x7A, 0xAE, 0x34, 0xCC, 0x2D, 0x47, 0x16, 0xDC, 0xA7, 0x98, 0xFD, 0xCF,
                0x66, 0xEE, 0xE0, 0x10,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x3A, 0x5B, 0x98, 0x43, 0xF0,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x3A, 0x60, 0xBE, 0x9E, 0x20,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x66, 0x72, 0x00, 0x02,
                0x00, 0x04, 0x00, 0x04, //domain
                0x63, 0x39, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
                0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x18, 0x00, 0x02, 0x00,
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00,
            },
            RawTicket = new byte[]
            {
                0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCC, //header
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x79, 0x3C, 0xF6, 0xFB, 0x7A, 0xAE, 0x34, 0xCC, 0x2D, 0x47, 0x16, 0xDC, 0xA7, 0x98, 0xFD, 0xCF,
                0x66, 0xEE, 0xE0, 0x10,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x3A, 0x5B, 0x98, 0x43, 0xF0,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x3A, 0x60, 0xBE, 0x9E, 0x20,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x66, 0x72, 0x00, 0x02,
                0x00, 0x04, 0x00, 0x04, //domain
                0x63, 0x39, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
                0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x18, 0x00, 0x02, 0x00,
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00,
                0x30, 0x02, 0x00, 0x20, //footer section
                0x00, 0x08, 0x00, 0x04, //key_id
                0x34, 0xCD, 0x3C, 0xA9,
                0x00, 0x08, 0x00, 0x14, //signature
            }
        };

        var ps3Ticket = new byte[]
        {
            0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCC, //header
            0x30, 0x00, 0x00, 0xA4, //body section
            0x00, 0x08, 0x00, 0x14, //serial_id
            0x79, 0x3C, 0xF6, 0xFB, 0x7A, 0xAE, 0x34, 0xCC, 0x2D, 0x47, 0x16, 0xDC, 0xA7, 0x98, 0xFD, 0xCF,
            0x66, 0xEE, 0xE0, 0x10,
            0x00, 0x01, 0x00, 0x04, //issuer_id
            0x00, 0x00, 0x01, 0x00,
            0x00, 0x07, 0x00, 0x08, //issue_date
            0x00, 0x00, 0x01, 0x3A, 0x5B, 0x98, 0x43, 0xF0,
            0x00, 0x07, 0x00, 0x08, //expire_date
            0x00, 0x00, 0x01, 0x3A, 0x60, 0xBE, 0x9E, 0x20,
            0x00, 0x02, 0x00, 0x08, //user_id
            0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
            0x00, 0x04, 0x00, 0x20, //username
            0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x04, //region/lang
            0x66, 0x72, 0x00, 0x02,
            0x00, 0x04, 0x00, 0x04, //domain
            0x63, 0x39, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x18, //service_id
            0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
            0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x01, 0x00, 0x04, //age/status
            0x18, 0x00, 0x02, 0x00,
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00,
            0x30, 0x02, 0x00, 0x20, //footer section
            0x00, 0x08, 0x00, 0x04, //key_id
            0x34, 0xCD, 0x3C, 0xA9,
            0x00, 0x08, 0x00, 0x14, //signature
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00
        };


        //Act
        var ticketDecoder = new Ps3TicketParser();
        var decodedTicket = ticketDecoder.ParseTicket(ps3Ticket);


        //Assert
        decodedTicket.Should().BeEquivalentTo(trueTicket);
    }

    [Fact]
    public void Parse_RPCN_Ticket()
    {
        var trueTicket = new Ps3Ticket
        {
            Header = new TicketHeader
            {
                MajorVersion = 2,
                MinorVersion = 1
            },
            Body = new TicketBody
            {
                Length = 0xA4,
                SerialId = new byte[]
                {
                    0x33, 0x32, 0x31, 0x33, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00
                },
                IssuerId = 0x33333333,
                IssuedDate = 1601434300267,
                ExpireDate = 1601435200267,
                UserId = 123456,
                Username = "testuser",
                Region = "br",
                Language = 0,
                Domain = "un",
                ServiceId = new byte[]
                {
                    0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34,
                    0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00
                },
                Status = 0,
                DateOfBirth = null!,
                Age = 0
            },
            Footer = new TicketFooter
            {
                KeyId = new byte[] {0x00, 0x00, 0x00, 0x00},
                Signature = new byte[]
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
                }
            },
            RawBody = new byte[]
            {
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x33, 0x32, 0x31, 0x33, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x33, 0x33, 0x33, 0x33,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0xEC, 0x2F, 0x6B,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0xF9, 0xEB, 0x0B,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x62, 0x72, 0x00, 0x00,
                0x00, 0x04, 0x00, 0x04, //domain
                0x75, 0x6E, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
                0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, //status_duration?
                0x00, 0x00, 0x00, 0x00, //date_of_birth
            },
            RawTicket = new byte[]
            {
                0x21, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, //header
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x33, 0x32, 0x31, 0x33, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x33, 0x33, 0x33, 0x33,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0xEC, 0x2F, 0x6B,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0xF9, 0xEB, 0x0B,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x62, 0x72, 0x00, 0x00,
                0x00, 0x04, 0x00, 0x04, //domain
                0x75, 0x6E, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
                0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, //status_duration?
                0x00, 0x00, 0x00, 0x00, //date_of_birth
                0x30, 0x02, 0x00, 0x44, //footer section
                0x00, 0x08, 0x00, 0x04, //key_id
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x38, //signature
            }
        };

        var ps3Ticket = new byte[]
        {
            0x21, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, //header
            0x30, 0x00, 0x00, 0xA4, //body section
            0x00, 0x08, 0x00, 0x14, //serial_id
            0x33, 0x32, 0x31, 0x33, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00,
            0x00, 0x01, 0x00, 0x04, //issuer_id
            0x33, 0x33, 0x33, 0x33,
            0x00, 0x07, 0x00, 0x08, //issue_date
            0x00, 0x00, 0x01, 0x74, 0xDC, 0xEC, 0x2F, 0x6B,
            0x00, 0x07, 0x00, 0x08, //expire_date
            0x00, 0x00, 0x01, 0x74, 0xDC, 0xF9, 0xEB, 0x0B,
            0x00, 0x02, 0x00, 0x08, //user_id
            0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
            0x00, 0x04, 0x00, 0x20, //username
            0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x04, //region/lang
            0x62, 0x72, 0x00, 0x00,
            0x00, 0x04, 0x00, 0x04, //domain
            0x75, 0x6E, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x18, //service_id
            0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
            0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x01, 0x00, 0x04, //age/status
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, //status_duration?
            0x00, 0x00, 0x00, 0x00, //date_of_birth
            0x30, 0x02, 0x00, 0x44, //footer section
            0x00, 0x08, 0x00, 0x04, //key_id
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x38, //signature
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00
        };


        //Act
        var ticketDecoder = new Ps3TicketParser();
        var decodedTicket = ticketDecoder.ParseTicket(ps3Ticket);


        //Assert
        decodedTicket.Should().BeEquivalentTo(trueTicket);
    }

    [Fact]
    public void Parse_PSN_V30_Ticket()
    {
        var trueTicket = new Ps3Ticket
        {
            Header = new TicketHeader
            {
                MajorVersion = 3,
                MinorVersion = 0
            },
            Body = new TicketBody
            {
                Length = 0xA4,
                SerialId = new byte[]
                {
                    0x02, 0xD6, 0x5A, 0x9A, 0xD8, 0x1D, 0x09, 0x34, 0x27, 0x42, 0x4B, 0x9B, 0x67, 0x29, 0x3E, 0xF2,
                    0x97, 0x55, 0xBC, 0x78
                },
                IssuerId = 0x100,
                IssuedDate = 1601425283268,
                ExpireDate = 1601425883000,
                UserId = 123456,
                Username = "testuser",
                Region = "us",
                Language = 1,
                Domain = "b6",
                ServiceId = new byte[]
                {
                    0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34,
                    0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00
                },
                DateOfBirth = new DateOfBirth
                {
                    Year = 1981,
                    Month = 2,
                    Day = 29
                },
                Age = 39,
                Status = 2
            },
            Footer = new TicketFooter
            {
                KeyId = new byte[] {0x38, 0x2D, 0xE5, 0x8D},
                Signature = new byte[]
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
                }
            },
            RawBody = new byte[]
            {
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x02, 0xD6, 0x5A, 0x9A, 0xD8, 0x1D, 0x09, 0x34, 0x27, 0x42, 0x4B, 0x9B, 0x67, 0x29, 0x3E, 0xF2, 0x97,
                0x55, 0xBC, 0x78,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0x62, 0x98, 0xC4,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0x6B, 0xBF, 0x78,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x75, 0x73, 0x00, 0x01,
                0x00, 0x04, 0x00, 0x04, //domain
                0x62, 0x36, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
                0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x11, 0x00, 0x04, //date_of_birth
                0x07, 0xBD, 0x02, 0x1D,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x27, 0x00, 0x02, 0x00,
            },
            RawTicket = new byte[]
            {
                0x31, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8, //header
                0x30, 0x00, 0x00, 0xA4, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0x02, 0xD6, 0x5A, 0x9A, 0xD8, 0x1D, 0x09, 0x34, 0x27, 0x42, 0x4B, 0x9B, 0x67, 0x29, 0x3E, 0xF2, 0x97,
                0x55, 0xBC, 0x78,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0x62, 0x98, 0xC4,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x74, 0xDC, 0x6B, 0xBF, 0x78,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x75, 0x73, 0x00, 0x01,
                0x00, 0x04, 0x00, 0x04, //domain
                0x62, 0x36, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
                0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x11, 0x00, 0x04, //date_of_birth
                0x07, 0xBD, 0x02, 0x1D,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x27, 0x00, 0x02, 0x00,
                0x30, 0x10, 0x00, 0x00, //unknown
                0x00, 0x00, 0x00, 0x00,
                0x30, 0x02, 0x00, 0x44, //footer section
                0x00, 0x08, 0x00, 0x04, //key_id
                0x38, 0x2D, 0xE5, 0x8D,
                0x00, 0x08, 0x00, 0x38, //signature
            },
        };

        var ps3Ticket = new byte[]
        {
            0x31, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8, //header
            0x30, 0x00, 0x00, 0xA4, //body section
            0x00, 0x08, 0x00, 0x14, //serial_id
            0x02, 0xD6, 0x5A, 0x9A, 0xD8, 0x1D, 0x09, 0x34, 0x27, 0x42, 0x4B, 0x9B, 0x67, 0x29, 0x3E, 0xF2, 0x97,
            0x55, 0xBC, 0x78,
            0x00, 0x01, 0x00, 0x04, //issuer_id
            0x00, 0x00, 0x01, 0x00,
            0x00, 0x07, 0x00, 0x08, //issue_date
            0x00, 0x00, 0x01, 0x74, 0xDC, 0x62, 0x98, 0xC4,
            0x00, 0x07, 0x00, 0x08, //expire_date
            0x00, 0x00, 0x01, 0x74, 0xDC, 0x6B, 0xBF, 0x78,
            0x00, 0x02, 0x00, 0x08, //user_id
            0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xE2, 0x40,
            0x00, 0x04, 0x00, 0x20, //username
            0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x04, //region/lang
            0x75, 0x73, 0x00, 0x01,
            0x00, 0x04, 0x00, 0x04, //domain
            0x62, 0x36, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x18, //service_id
            0x55, 0x50, 0x30, 0x30, 0x30, 0x36, 0x2D, 0x42, 0x4C, 0x55, 0x53, 0x33, 0x30, 0x34, 0x36, 0x34, 0x5F,
            0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x30, 0x11, 0x00, 0x04, //date_of_birth
            0x07, 0xBD, 0x02, 0x1D,
            0x00, 0x01, 0x00, 0x04, //age/status
            0x27, 0x00, 0x02, 0x00,
            0x30, 0x10, 0x00, 0x00, //unknown
            0x00, 0x00, 0x00, 0x00,
            0x30, 0x02, 0x00, 0x44, //footer section
            0x00, 0x08, 0x00, 0x04, //key_id
            0x38, 0x2D, 0xE5, 0x8D,
            0x00, 0x08, 0x00, 0x38, //signature
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        };


        //Act
        var ticketDecoder = new Ps3TicketParser();
        var decodedTicket = ticketDecoder.ParseTicket(ps3Ticket);


        //Assert
        decodedTicket.Should().BeEquivalentTo(trueTicket);
    }

    [Fact]
    public void Parse_PSN_V40_Ticket()
    {
        var trueTicket = new Ps3Ticket
        {
            Header = new TicketHeader
            {
                MajorVersion = 4,
                MinorVersion = 0
            },
            Body = new TicketBody
            {
                Length = 0x100,
                SerialId = new byte[]
                {
                    0xAF, 0x67, 0x48, 0xE7, 0x27, 0x2A, 0x3B, 0x13, 0x62, 0x9E, 0xDF, 0xFC, 0xC5, 0xAB, 0x05, 0x9B,
                    0x7B, 0x92, 0xFA, 0x02
                },
                IssuerId = 0x00000100,
                IssuedDate = 1350155388701,
                ExpireDate = 1350241788000,
                UserId = 6426198810416408665,
                Username = "testuser",
                Region = "fr",
                Language = 2,
                Domain = "c9",
                ServiceId = new byte[]
                {
                    0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
                    0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00
                },
                DateOfBirth = new DateOfBirth
                {
                    Year = 1988,
                    Month = 2,
                    Day = 29
                },
                Age = 24,
                Status = 2,
            },
            Footer = new TicketFooter
            {
                KeyId = new byte[] { 0x34, 0xCD, 0x3C, 0xA9 },
                Signature = new byte[]
                {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
                }
            },
            RawBody = new byte[]
            {
                0x30, 0x00, 0x01, 0x00, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0xAF, 0x67, 0x48, 0xE7, 0x27, 0x2A, 0x3B, 0x13, 0x62, 0x9E, 0xDF, 0xFC, 0xC5, 0xAB, 0x05, 0x9B,
                0x7B, 0x92, 0xFA, 0x02,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x3A, 0x5B, 0x88, 0x47, 0x1D,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x3A, 0x60, 0xAE, 0xA0, 0x60,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x66, 0x72, 0x00, 0x02,
                0x00, 0x04, 0x00, 0x04, //domain
                0x63, 0x39, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
                0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x11, 0x00, 0x04, //date_of_birth
                0x07, 0xC4, 0x02, 0x1D,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x18, 0x00, 0x02, 0x00,
                0x30, 0x10, 0x00, 0x00, //unknown
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x08, //unknown string one (8)
                0x50, 0x53, 0x33, 0x5f, 0x43, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x40, //unknown string two (64)
                0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            },
            RawTicket = new byte[]
            {
                0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x34, //header
                0x30, 0x00, 0x01, 0x00, //body section
                0x00, 0x08, 0x00, 0x14, //serial_id
                0xAF, 0x67, 0x48, 0xE7, 0x27, 0x2A, 0x3B, 0x13, 0x62, 0x9E, 0xDF, 0xFC, 0xC5, 0xAB, 0x05, 0x9B,
                0x7B, 0x92, 0xFA, 0x02,
                0x00, 0x01, 0x00, 0x04, //issuer_id
                0x00, 0x00, 0x01, 0x00,
                0x00, 0x07, 0x00, 0x08, //issue_date
                0x00, 0x00, 0x01, 0x3A, 0x5B, 0x88, 0x47, 0x1D,
                0x00, 0x07, 0x00, 0x08, //expire_date
                0x00, 0x00, 0x01, 0x3A, 0x60, 0xAE, 0xA0, 0x60,
                0x00, 0x02, 0x00, 0x08, //user_id
                0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
                0x00, 0x04, 0x00, 0x20, //username
                0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x04, //region/lang
                0x66, 0x72, 0x00, 0x02,
                0x00, 0x04, 0x00, 0x04, //domain
                0x63, 0x39, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x18, //service_id
                0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
                0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x11, 0x00, 0x04, //date_of_birth
                0x07, 0xC4, 0x02, 0x1D,
                0x00, 0x01, 0x00, 0x04, //age/status
                0x18, 0x00, 0x02, 0x00,
                0x30, 0x10, 0x00, 0x00, //unknown
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x08, //unknown string one (8)
                0x50, 0x53, 0x33, 0x5f, 0x43, 0x00, 0x00, 0x00,
                0x00, 0x08, 0x00, 0x40, //unknown string two (64)
                0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x30, 0x02, 0x00, 0x2C, //footer section
                0x00, 0x08, 0x00, 0x04, //key_id
                0x34, 0xCD, 0x3C, 0xA9,
                0x00, 0x08, 0x00, 0x20, //signature
            }
        };

        var ps3Ticket = new byte[]
        {
            0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x34, //header
            0x30, 0x00, 0x01, 0x00, //body section
            0x00, 0x08, 0x00, 0x14, //serial_id
            0xAF, 0x67, 0x48, 0xE7, 0x27, 0x2A, 0x3B, 0x13, 0x62, 0x9E, 0xDF, 0xFC, 0xC5, 0xAB, 0x05, 0x9B,
            0x7B, 0x92, 0xFA, 0x02,
            0x00, 0x01, 0x00, 0x04, //issuer_id
            0x00, 0x00, 0x01, 0x00,
            0x00, 0x07, 0x00, 0x08, //issue_date
            0x00, 0x00, 0x01, 0x3A, 0x5B, 0x88, 0x47, 0x1D,
            0x00, 0x07, 0x00, 0x08, //expire_date
            0x00, 0x00, 0x01, 0x3A, 0x60, 0xAE, 0xA0, 0x60,
            0x00, 0x02, 0x00, 0x08, //user_id
            0x59, 0x2E, 0x71, 0xC5, 0x46, 0xE8, 0x68, 0x59,
            0x00, 0x04, 0x00, 0x20, //username
            0x74, 0x65, 0x73, 0x74, 0x75, 0x73, 0x65, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x04, //region/lang
            0x66, 0x72, 0x00, 0x02,
            0x00, 0x04, 0x00, 0x04, //domain
            0x63, 0x39, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x18, //service_id
            0x49, 0x56, 0x30, 0x30, 0x30, 0x31, 0x2D, 0x4E, 0x50, 0x58, 0x53, 0x30, 0x31, 0x30, 0x30, 0x31,
            0x5F, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x30, 0x11, 0x00, 0x04, //date_of_birth
            0x07, 0xC4, 0x02, 0x1D,
            0x00, 0x01, 0x00, 0x04, //age/status
            0x18, 0x00, 0x02, 0x00,
            0x30, 0x10, 0x00, 0x00, //unknown
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x08, //unknown string one (8)
            0x50, 0x53, 0x33, 0x5f, 0x43, 0x00, 0x00, 0x00,
            0x00, 0x08, 0x00, 0x40, //unknown string two (64)
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x30, 0x02, 0x00, 0x2C, //footer section
            0x00, 0x08, 0x00, 0x04, //key_id
            0x34, 0xCD, 0x3C, 0xA9,
            0x00, 0x08, 0x00, 0x20, //signature
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        };


        //Act
        var ticketDecoder = new Ps3TicketParser();
        var decodedTicket = ticketDecoder.ParseTicket(ps3Ticket);


        //Assert
        decodedTicket.Should().BeEquivalentTo(trueTicket);
    }
}